...
// 시·도 뒤의 시·군·구 — 「서울 강남구」→「강남구」, 「경기 성남시 분당구」→「성남시 분당구」.
// 동 단위는 쓰지 않는다: 고용24 주소가 도로명이라 동 이름이 일부 주소에만 있다(2026-09-13 확인).
function guOf(region) {
  return (region || '').split(' ').slice(1).join(' ') || null;
}

const decode =s => (s || '').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"').replace(/&#39;/g, "'").trim();
const esc = s => (s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');

module.exports = { SIDO_SLUG, SIDO_NAME, sidoOf, guOf, decode, esc };
